...
public class RegClient {
	...
	private static final Logger logger = Logger.getLogger(RegClient.class);
			...
			boot.group(group).channel(NioSocketChannel.class).option(ChannelOption.TCP_NODELAY, true)
					.handler(new ChannelInitializer<SocketChannel>() {
						public void initChannel(SocketChannel ch) {
							...
							ch.pipeline().addLast(new ProtobufDecoder(RegRespProto.RegResp.getDefaultInstance()));
							ch.pipeline().addLast(new ProtobufVarint32LengthFieldPrepender());
							ch.pipeline().addLast(new ProtobufEncoder());
							ch.pipeline().addLast(new ClientRegHandler());
						}
					});
			try {
